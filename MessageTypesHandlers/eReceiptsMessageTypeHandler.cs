...
{
    /// <summary>
    /// Handler class for eReceipts emails.
    /// </summary>
    public sealed class EReceiptsMessageTypeHandler : IMessageTypeHandler
    {
        /// <summary>
        /// handel method that brings the map, getData and arguments together for eReceipts.
        /// </summary>
        ...
        private object ParseMessageArguments(Message message)
        {
            ...
            var result = new EReceiptsMessageArgs();
        ...
        private static string ConvertBarCodeURL(string strInput)
        {

            string[] strSeparators = new string[] { "<BARCODE>", "</BARCODE>" };
            string[] strSplit = strInput.Split(strSeparators, StringSplitOptions.None);
            for (int i = 0; i < strSplit.Length; i++)
            {
                Regex regex = new Regex(@"^\d+$");
                if (regex.IsMatch(strSplit[i].Trim()))
                    strSplit[i] = string.Format(ConfigurationManager.AppSettings["BarcodeURL"], strSplit[i].Trim());

            }
            return "<![CDATA[" + String.Join("", strSplit) + "]]>";
        }

        private static string EscapeEReceiptsFormat(string str)
        {
            string retString = string.Empty;
            retString = Regex.Replace(str, @"{GRAPHIC=(.)*?bmp}\n", "");
            retString = Regex.Replace(str, @"{GRAPHIC=(.)*?bmp}", "");
            return retString;
        }
